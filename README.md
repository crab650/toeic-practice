...
- 練習紀錄可靠同步：四種練習會以 UUID 事件保存於本機佇列，可手動批次同步至 Personal Learning & Workspace 平台。
...
每次完成句子拼寫、句子重組、TOEIC Part 5 或 TOEIC Part 2 練習時，程式會先將完整事件寫入本機 `practice_events`，狀態為 `pending`。右上角「同步」按鈕會先透過平台 `/api/v1/auth/me` 確認帳號，再將資料以每批最多 100 筆送到 `/api/v1/study/practice-logs`。

平台 Base URL 可在設定視窗修改。Windows 正式版的 Token 保存在 Windows Credential Manager，不會寫入 `config.json` 或 SQLite。開發及自動化測試也可使用：

```powershell
$env:LEARNING_PLATFORM_BASE_URL="http://127.0.0.1:5050"
$env:LEARNING_PLATFORM_TOKEN="your-platform-token"
```

目前平台接收端尚未建立時，本機仍會安全累積 `pending` 紀錄；待 API 與 Token 可用後再按「同步」即可。

## Recent Updates (最近更新)

### 2026-07-12
- **越南語 (Tiếng Việt) 語系支援**：
  - 新增「🇬🇧 EN / 🇻🇳 VI」語系切換按鈕，支援在多個語系單元之間自由切換與導入。
  - 新增越文專屬 AI 句子解析 Prompt（支援量詞、人稱代名詞與時態等繁體中文講解）。
- **學習報表與 GitHub 風格熱力圖**：
  - 右上角新增「報表」視窗，動態繪製最近 6 個月的每日練習熱力圖，視覺化呈現學習進度。
  - 新增每日練習明細表，支援分開展示英文與越文的練習次數、答錯次數、播音次數與偷看答案記錄。
- **單字發音輔助系統 (越文模式)**：
  - **拼寫模式 (Spelling Mode)**：輸入框上方新增微型發音按鈕，聚焦或懸停時點擊發音且不搶走焦點。
  - **重組模式 (Scramble Mode)**：單字選項卡片與上方的已選卡片新增小喇叭發音圖示，支援發音阻斷點擊，不破壞重組流程。
  - **點擊單字發音與 AI 查詢**：正確答案句中的所有單字轉為可點擊標籤，點擊直接播放單字發音，並自動填入側邊欄觸發 Gemini AI 查詢其 IPA 音標、聲調與中文含意。
  - **AI 句子解析越文優化**：
    - 將解析介面的標籤繁體中文化（中文翻譯、語法與代名詞解析、核心單字與片語等）。
    - 優化 Prompt 機制：強制 AI 列出越南語句子中的「每一個單字」做逐字中文解釋，避免遺漏。
    - 前端新增 **「🔄（重新整理）」** 按鈕，支援無視快取強行重新向 Gemini API 獲取最新的解析內容並更新資料庫。
- **學習平台練習紀錄可靠同步**：
  - 新增本機練習紀錄 UUID 事件化機制，將 `spelling`、`scramble`、`toeic_part5` 與 `toeic_part2` 四種練習類型之作答結果、撥音次數、是否偷看答案等欄位，在完成時以唯一 `event_id` 保存至 SQLite `practice_events` 資料表。
  - 支援手動批次同步：右上角新增「同步」按鈕，可將待同步練習紀錄批次（每批最多 100 筆）上傳至個人學習平台，使用 `event_id` 確保上傳去重，並依實際練習時間歸檔。
  - 安全 Token 管理：Bearer Token 儲存於 Windows Credential Manager，絕不寫入設定檔或資料庫，且支援環境變數（`LEARNING_PLATFORM_TOKEN`）覆寫。
  - 設定視窗介面擴充：設定 modal 中新增學習平台 Base URL 與 Token 設定，並提供待同步、失敗、上次成功時間與裝置 ID 等同步狀態詳細資訊。
  - 提供轉換工具：新增 `tools/backfill_practice_history.py` 腳本，可單次冪等轉換既有歷史練習紀錄至同步事件佇列。
