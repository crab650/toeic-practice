...
目前後端是單檔式架構，對小型本機工具來說可維護，但未來功能變多時建議拆分。
...
`app.py` 與 `app.js` 都承擔太多責任。短期可接受，長期會增加修改風險。
...
目前資料表由 `app.py` 內的 `CREATE TABLE` 和 `ALTER TABLE` 管理。未來欄位增加時，建議導入簡單 migration 機制。
...
4. 補最小 API 測試
5. 拆分 `app.py` 中的 Gemini 與 TTS service
6. 拆分 `app.js` 的模式邏輯
